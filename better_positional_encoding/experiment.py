...
class PixelCoordinateEmbeddings(nn.Module):
    def __init__(self, embedding_size):
        ...
        with torch.no_grad():
            self.B.weight[:(embedding_size-2-1)//2,0] = torch.arange(0,(embedding_size-2-1)//2)
            self.B.weight[(embedding_size-2-1)//2:,1] = torch.arange(0,(embedding_size-2-1)-(embedding_size-2-1)//2)
...
model = Coordinet(input_dhw, 30, 20, 10, [200,],[200,])
